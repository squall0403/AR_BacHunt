...
public class xmlReader_Level_Intro_1 : MonoBehaviour {
	...
	void Update(){
		...
		language [currentLanguage].TryGetValue ("intro_level1", out txt_fact);
		language [currentLanguage].TryGetValue ("guide_Lv1", out txt_guide);
	}
	...
	void Reader(){
		...
		foreach(XmlNode languageValue in languageList){
			...
			foreach(XmlNode value in languageContent){
				...
				if(value.Name == "level_intro" || value.Name == "intro_level1" || value.Name == "guide_Lv1" || value.Name == "btn_Play")
					obj.Add (value.Name, value.InnerText);
