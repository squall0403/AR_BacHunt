...
public class xmlReader_Level_Intro_3 : MonoBehaviour {
	...
	void Update(){
		...
		language [currentLanguage].TryGetValue ("intro_level3", out txt_fact);
		language [currentLanguage].TryGetValue ("guide_Lv3", out txt_guide);
	}
	...
	void Reader(){
		...
		foreach(XmlNode languageValue in languageList){
			...
			foreach(XmlNode value in languageContent){
				...
				if(value.Name == "level_intro" || value.Name == "intro_level3" || value.Name == "guide_Lv3" || value.Name == "btn_Play")
					obj.Add (value.Name, value.InnerText);
